...
## 3. 주요 분석 프로세스
### 1) 데이터 전처리 및 분석
- **Unix Time 변환**: 로그의 선후 관계 및 시청 시간 계산을 위해 시각 데이터를 수치형 데이터로 변환
- **이상 로그 제거**: 시청 시작 시간이 종료 시간보다 늦게 기록된 기계적 오류 데이터 필터링
- **로그 묶음 처리**: 동일 세션 내 발생하는 중간 로그를 통합하여 단일 시청 형태로 구조화

### 2) 편집규칙 적용
- **비정상 종료 보정**: 종료 시점이 누락된 로그를 감지하여 후속 로그 발생 시점에 맞춰 종료 시간 정밀 보정
...
- **비정상 로그의 패턴화**: 비정상 종료 로그가 특정 시간 간격으로 발생함을 데이터로 증명하여 보정 로직의 객관적 근거 확보
- **시각화 기반 검증**: ID별 간트 차트를 생성하여 원천 데이터가 가공 단계(LV1→LV2→LV3→NEW)를 거치며 정제되는 과정을 직관적으로 비교

<br>

## 5. 분석 결과 시각화
| ID별 시청 로그 단계별 비교 (Gantt) |
| :---: |
| <img src="https://github.com/user-attachments/assets/a1a4e1af-8800-4590-992c-34872906089f" width="1000" /> |
| *LV1→LV2→LV3→NEW 단계별 정제 과정* |
